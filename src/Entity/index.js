...
const Card = ({ title, description, type, setIsOpen, id, fetchData }) => {
  const handleDelete = () => {
    axios
      .delete(`http://localhost:3000/entity/${id}`)
      .then((res) => {
        console.log(res);
        fetchData();
        setIsOpen(false);
  ...
  const handleClick = (id) => {
    console.log(id);
    // navigate
    loc({ pathname: "/form", search: `?id=${id}` });
  };
...
const Entity = () => {
  ...
  const fetchData = () => {
    axios
      .get("http://localhost:3000/entity?moduleId=" + id)
      .then(({ data }) => {
        if (data) {
          setCardData(data);
          console.log(data);
  ...
  return (
    <div>
      ...
      <Modal isOpen={isOpen} onClose={toggleModal} children={<AddEntity />} />
    </div>
  );
};
const AddEntity = () => {
  ...
  const id = location.search.split("=")[1];
      ...
      axios
        .post("http://localhost:3000/entity", payload)
        .then((res) => {
          console.log(res);
