...
export default function Multiselect({inputs ,onChange}) {
  ...
  return (
   ...
   <label className="block mb-2 mt-4 text-lg font-medium text-gray-900 font-bold">{inputs.title}</label>
    {options.map((option) => (
      <div key={option.id}  className="px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-indigo-500">
        <input
          type="checkbox"
