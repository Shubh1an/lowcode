...
import CustomInput from '../../Components/inputs/CustomInput';
    const Column = ({ columnfeilds, tasks, setTasks,color ,setcolumnfeilds,onChange}) => {
    ...
    return (
        ...
        onDrop={(e) => onDrop(e, columnfeilds)}>
            {
                isEdit  ?  <CustomInput inputs={ {
                    title: columnfeilds,
                    type: "text",
                    id: "title",
                    
                }}  onChange={onChange} /> :
                <h2 className={`bg-[${color}] p-2 text-lg font-semibold mb-4 flex gap-1  items-center justify-center`}>{columnfeilds} <AiFillEdit  onClick={() => isEditable(isEdit)} /> </h2>
            }
      
       <div className="bg-gray-200 p-4 rounded-md w-80">
           {tasks.filter(task => task.status === columnfeilds).map(task => (
